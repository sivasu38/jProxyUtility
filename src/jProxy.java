...
public class jProxy extends Thread
{
	...
	static ArrayList<String> logentries = new ArrayList<>();
	static HashMap<String ,ArrayList<Object>> callResults = new HashMap<>();  
...
//		port = Integer.parseInt(args[0]);
	...
	public static void storeAllCalls(String request, String timeelapsed, int responseCode){
		ArrayList<Object> allCalls = new ArrayList<>();
		try
		{
		  if(!request.isEmpty())
		  allCalls.add(responseCode);
		  allCalls.add(timeelapsed);
		  callResults.put(request, allCalls);
//		  allCalls.removeAll(allCalls);
		}
		catch(Exception e)
		{
			try{allCalls.removeAll(allCalls);} catch(Exception e1){}
		}
	}
...
class ProxyThread extends Thread
{
	...
	public static StringBuffer networkURL = new StringBuffer("");
	...
	public void run()
	{
		try
		{
			...
			if (debugLevel == 1)
			{
				...
				debugOut.println("REQUEST:\n" + (new String(request)));
				debugOut.println("RESPONSE:\n" + (new String(response)));
				jProxy.storeAllCalls(networkURL.toString(),Long.toString(endTime - startTime),responseCode);
				debugOut.flush();
			}
...
//		int responseCode = 200;
		...
		try
		{
			...
			if (data != null)
			{
				header.append(data + "\r\n");
				pos = data.indexOf(" ");
				if(data.contains("GET") || data.contains("CONNECT") || data.contains("POST"))
				{
					networkURL = networkURL.append(data);
				}
